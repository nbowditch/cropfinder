function cropfinder_calc_HOGs( image_dir, feat_dir, metadata_dir, ...
    label_dir, score_dir, category)
%CROPFINDER_CALC_HOGS Summary of this function goes here
...
for idInd = 1:limit
    ...
    for subInd = 3:(num_subfolders + 2)
        ...
        if exist(fullname{1}, 'file')
            ...
            feats{idInd} = extractHOGFeatures(im, 'CellSize', ...
                [cellH cellW], 'BlockSize', [2 2], 'BlockOverlap', ...
                [0 0], 'NumBins', 4);
            %fprintf('      HOG %i of %i, size is %i by %i\n', idInd, ...
